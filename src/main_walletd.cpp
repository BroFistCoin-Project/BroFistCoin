...
Options:
  ...
  --brofistcoind-remote-address=<ip:port> Connect to remote brofistcoind and suppress running built-in brofistcoind.
  --brofistcoind-authorization=<auth>     HTTP Basic Authorization header (base64 of login:password)

Options for built-in brofistcoind (run when no --brofistcoind-remote-address specified):
  ...
  --brofistcoind-bind-address=<ip:port>   Interface and port for brofistcoind RPC [default: 0.0.0.0:8081].
...
static const bool separate_thread_for_brofistcoind = true;
...
int main(int argc, const char *argv[]) try {
	...
	brofistcoin::Config config(cmd);
	brofistcoin::Currency currency(config.is_testnet);

	if (cmd.should_quit(USAGE, brofistcoin::app_version()))
		return 0;
	logging::LoggerManager logManagerNode;
	logManagerNode.configure_default(config.get_coin_directory("logs"), "brofistcoind-");
	...
	try {
		if (!config.brofistcoind_remote_port)
			blockchain_lock = std::make_unique<platform::ExclusiveLock>(coinFolder, "brofistcoind.lock");
	} catch (const platform::ExclusiveLock::FailedToLock &ex) {
		std::cout << ex.what() << std::endl;
		return api::BROFISTCOIND_ALREADY_RUNNING;
	}
	try {
	...
	std::thread brofistcoind_thread;
	if (!config.brofistcoind_remote_port) {
		try {
			if (separate_thread_for_brofistcoind) {
				brofistcoind_thread = std::thread([&prm, &logManagerNode, &config, &currency] {
					boost::asio::io_service io;
			...
			std::cout << ex.what() << std::endl;
			if (brofistcoind_thread.joinable())
				brofistcoind_thread.join();  // otherwise terminate will be called in ~thread
			return api::BROFISTCOIND_BIND_PORT_IN_USE;
		}
	}
