...
crypto::Hash brofistcoin::proof_of_trust::get_hash() const {
	std::string s;
...
// P2pProtocolTypes
void ser(brofistcoin::UUID &v, seria::ISeria &s) { s.binary(&v, sizeof(v)); }

void ser_members(brofistcoin::PeerlistEntry &v, seria::ISeria &s) {
...
void ser_members(brofistcoin::NetworkAddress &v, seria::ISeria &s) {
	seria_kv("ip", v.ip, s);
	seria_kv("port", v.port, s);
}

void ser_members(brofistcoin::connection_entry &v, seria::ISeria &s) {
	seria_kv("adr", v.adr, s);
	seria_kv("id", v.id, s);
	seria_kv("is_income", v.is_income, s);
}

void ser_members(brofistcoin::CoreStatistics &v, seria::ISeria &s) {
...
void ser_members(brofistcoin::COMMAND_REQUEST_STAT_INFO::request &v, seria::ISeria &s) { seria_kv("tr", v.tr, s); }
...
void ser_members(brofistcoin::COMMAND_REQUEST_NETWORK_STATE::request &v, seria::ISeria &s) { seria_kv("tr", v.tr, s); }
...
void ser_members(brofistcoin::RawBlockLegacy &v, seria::ISeria &s) {
	...
	if (s.is_input()) {
		...
		std::transform(other_transactions.begin(), other_transactions.end(), std::back_inserter(v.transactions),
		    [](const std::string &s) { return brofistcoin::BinaryArray(s.data(), s.data() + s.size()); });
	} else {
		...
		std::transform(v.transactions.begin(), v.transactions.end(), std::back_inserter(other_transactions),
		    [](brofistcoin::BinaryArray &s) { return std::string(s.begin(), s.end()); });
		seria::seria_kv("block", other_block, s);
		seria::seria_kv("txs", other_transactions, s);
	}
}

void ser_members(brofistcoin::NOTIFY_NEW_BLOCK::request &v, seria::ISeria &s) {
	seria_kv("b", v.b, s);
	seria_kv("current_blockchain_height", v.current_blockchain_height, s);
	seria_kv("hop", v.hop, s);
}

void ser_members(brofistcoin::NOTIFY_NEW_TRANSACTIONS::request &v, seria::ISeria &s) {
	...
	if (s.is_input()) {
		...
		std::transform(v.txs.begin(), v.txs.end(), std::back_inserter(transactions),
		    [](const brofistcoin::BinaryArray &s) { return std::string(s.begin(), s.end()); });
		seria::seria_kv("txs", transactions, s);
	}
}

void ser_members(brofistcoin::NOTIFY_REQUEST_GET_OBJECTS::request &v, seria::ISeria &s) {
	serialize_as_binary(v.txs, "txs", s);
	serialize_as_binary(v.blocks, "blocks", s);
}

void ser_members(brofistcoin::NOTIFY_RESPONSE_GET_OBJECTS::request &v, seria::ISeria &s) {
...
void ser_members(brofistcoin::NOTIFY_REQUEST_CHAIN::request &v, seria::ISeria &s) {
	serialize_as_binary(v.block_ids, "block_ids", s);
}

void ser_members(brofistcoin::NOTIFY_RESPONSE_CHAIN_ENTRY::request &v, seria::ISeria &s) {
...
void ser_members(brofistcoin::NOTIFY_REQUEST_TX_POOL::request &v, seria::ISeria &s) {
	serialize_as_binary(v.txs, "txs", s);
}
}
