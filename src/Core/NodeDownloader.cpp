...
void Node::DownloaderV11::on_connect(P2PClientBrofistcoin *who) {
...
void Node::DownloaderV11::on_disconnect(P2PClientBrofistcoin *who) {
	if (who->is_incoming())
		return;
...
void Node::DownloaderV11::on_msg_notify_request_chain(P2PClientBrofistcoin *who,
...
void Node::DownloaderV11::advance_chain() {
	if (m_chain_client || !m_chain.empty())
		return;
	std::vector<P2PClientBrofistcoin *> lagging_clients;
	std::vector<P2PClientBrofistcoin *> sorted_clients;
	...
	std::sort(sorted_clients.begin(), sorted_clients.end(), [](P2PClientBrofistcoin *a, P2PClientBrofistcoin *b) -> bool {
...
void Node::DownloaderV11::on_msg_notify_request_objects(P2PClientBrofistcoin *who,
	...
	for (auto &&rb : req.blocks) {
		...
		try {
			...
			bid = brofistcoin::get_block_hash(bheader);
...
void Node::DownloaderV11::advance_download(Hash last_downloaded_block) {
	...
	std::map<P2PClientBrofistcoin *, size_t> who_downloaded_counter;
	...
	for (auto &&dc : m_download_chain) {
		...
		if (total_downloading_blocks >= TOTAL_DOWNLOAD_BLOCKS)
			break;
		P2PClientBrofistcoin *ready_client = nullptr;
...
void Node::DownloaderV1::on_connect(P2PClientBrofistcoin *who) {
...
void Node::DownloaderV1::on_disconnect(P2PClientBrofistcoin *who) {
	if (who->is_incoming())
		return;
	m_good_clients.erase(who);
...
void Node::DownloaderV1::advance_download(Hash last_downloaded_block) {
	...
	if (!m_sync_client) {  // start sync with some client who has better chain
		P2PClientBrofistcoin *worst_client = nullptr;
		for (auto &&who : m_good_clients) {
			if (!worst_client ||
...
void Node::DownloaderV1::on_msg_notify_request_chain(P2PClientBrofistcoin *who,
    const NOTIFY_RESPONSE_CHAIN_ENTRY::request &req) {
...
void Node::DownloaderV1::on_msg_notify_request_objects(P2PClientBrofistcoin *,
    const NOTIFY_RESPONSE_GET_OBJECTS::request &req) {
	if (!m_sync_sent)
		return;
