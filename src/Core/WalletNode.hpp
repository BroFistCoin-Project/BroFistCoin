...
public:
	...
	bool handle_send_transaction3(http::Client *, http::RequestData &&, json_rpc::Request &&,
	    api::brofistcoind::SendTransaction::Request &&,
	    api::brofistcoind::SendTransaction::Response &);  // We lock spent outputs until next pool sync
...
private:
	...
	struct LongPollClient {
		...
		http::RequestData original_request;
		json_rpc::OptionalJsonValue original_jsonrpc_id;
		brofistcoin::api::walletd::GetStatus::Request original_get_status;
	};
