...
public:
	...
	bool parse(const BinaryArray &extra) {
		m_fields.clear();
		return brofistcoin::parse_transaction_extra(extra, m_fields);
	}
	template<typename T>
	...
	bool get_public_key(crypto::PublicKey &pk) const {
		brofistcoin::TransactionExtraPublicKey extra_pk;
		if (!get(extra_pk)) {
			return false;
		}
...
private:
	std::vector<brofistcoin::TransactionExtraField>::const_iterator find(const std::type_info &t) const {
		return std::find_if(
		    m_fields.begin(), m_fields.end(), [&t](const brofistcoin::TransactionExtraField &f) { return t == f.type(); });
	}
	std::vector<brofistcoin::TransactionExtraField>::iterator find(const std::type_info &t) {
		return std::find_if(
		    m_fields.begin(), m_fields.end(), [&t](const brofistcoin::TransactionExtraField &f) { return t == f.type(); });
	}

	std::vector<brofistcoin::TransactionExtraField> m_fields;
};
}

namespace seria {
class ISeria;
void ser(brofistcoin::TransactionExtraMergeMiningTag &v, ISeria &s);
}
