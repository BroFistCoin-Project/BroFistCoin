...
static void parse_peer_and_add_to_container(std::string str, std::vector<brofistcoin::NetworkAddress> &container) {
	brofistcoin::NetworkAddress na{};
...
const static UUID BROFISTCOIN_NETWORK = {{0x40, 0x0a, 0x02, 0x00, 0x20, 0x1b, 0x1d, 0x7b, 0x0f, 0x11, 0x08, 0x00, 0x12,
    0x31, 0x02, 0x1d}};  // Bender's nightmare
...
Config::Config(common::CommandLine &cmd)
    ...
    , network_id(BROFISTCOIN_NETWORK)
    , p2p_bind_port(P2P_DEFAULT_PORT)
    , p2p_external_port(P2P_DEFAULT_PORT)
    , p2p_bind_ip("0.0.0.0")
    , brofistcoind_bind_port(RPC_DEFAULT_PORT)
    , brofistcoind_bind_ip("127.0.0.1")  // Less attack vectors from outside for ordinary uses
    , brofistcoind_remote_port(0)
    , brofistcoind_remote_ip("127.0.0.1")
	...
	if (const char *pa = cmd.get("--brofistcoind-authorization")) {
		// go to https://www.base64encode.org and encode username:password there
		brofistcoind_authorization = pa;
	}
	if (const char *pa = cmd.get("--brofistcoind-bind-address")) {
		if (!common::parse_ip_address_and_port(brofistcoind_bind_ip, brofistcoind_bind_port, pa))
			throw std::runtime_error("Wrong address format " + std::string(pa) + ", should be ip:port");
	}
	if (const char *pa = cmd.get("--brofistcoind-remote-address")) {
		const std::string prefix = "https://";
		if (std::string(pa).find(prefix) == 0) {
#if !BROFISTCOIN_SSL
			throw std::runtime_error("Using https in --brofistcoind-remote-address impossible - this binary is built without OpenSSL");
#endif
			std::string sip;
			std::string sport;
			...
			brofistcoind_remote_port = boost::lexical_cast<uint16_t>(sport);
			brofistcoind_remote_ip = prefix + sip;
		}else {
			if (!common::parse_ip_address_and_port(brofistcoind_remote_ip, brofistcoind_remote_port, pa))
				throw std::runtime_error("Wrong address format " + std::string(pa) + ", should be ip:port");
		}
	}
	...
	if (seed_nodes.empty() && !is_testnet)
		for (auto &&sn : brofistcoin::SEED_NODES) {
			NetworkAddress addr;
