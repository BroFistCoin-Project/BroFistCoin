...
bool WalletNode::on_api_http_request(http::Client *who, http::RequestData &&request, http::ResponseData &response) {
	...
	add_waiting_command(who, std::move(original_request), json_rpc::OptionalJsonValue{}, std::move(request),
	    [=](const WaitingClient &wc, http::ResponseData &&send_response) mutable {
		    ...
		    // brofistcoind never sends connection-close, so we are safe to retain all
		    // headers
...
bool WalletNode::handle_create_transaction3(http::Client *who, http::RequestData &&raw_request,
	...
	if (request.fee_per_byte == 0)
		throw json_rpc::Error(json_rpc::errInvalidParams,
		    ...
		    "because walletd never connected to brofistcoind after "
		    "it was restarted");
	...
	api::brofistcoind::GetRandomOutputs::Request ra_request;
	...
	api::brofistcoind::GetRandomOutputs::Response ra_response;
	if (m_inproc_node) {
	...
	http::RequestData new_request =
	    json_rpc::create_request(api::brofistcoind::url(), api::brofistcoind::GetRandomOutputs::method(), ra_request);
...
bool WalletNode::handle_send_transaction3(http::Client *who, http::RequestData &&raw_request,
    json_rpc::Request &&raw_js_request, api::brofistcoind::SendTransaction::Request &&request,
    api::brofistcoind::SendTransaction::Response &response) {
	Transaction tx;
	...
	new_request.r.set_firstline("POST", api::brofistcoind::url(), 1, 1);
