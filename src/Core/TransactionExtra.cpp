...
/*bool createTxExtraWithPaymentId(const std::string &paymentIdString, BinaryArray &extra) {
    ...
    brofistcoin::set_payment_id_to_transaction_extra_nonce(extraNonce, paymentIdBin);

    if (!brofistcoin::add_extra_nonce_to_transaction_extra(extra, extraNonce)) {
        return false;
    }
...
static void doSerialize(brofistcoin::TransactionExtraMergeMiningTag &tag, seria::ISeria &s) {
	s.begin_object();
...
void seria::ser(brofistcoin::TransactionExtraMergeMiningTag &v, ISeria &s) {
	if (s.is_input()) {
		std::string field;
		s(field);
